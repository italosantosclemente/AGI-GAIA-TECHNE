...
function generate_manifesto(decision::FirewallDecision, book::TraceBook)::BoundaryManifesto
    ...
    return BoundaryManifesto(
        now(),
        decision.verdict,
        decision.audit_pattern,
        thesis,
        antithesis,
        btype,
        length(book.entries),
        decision.severity,
        decision.requires_reflexive_judgment,
        pause_active,
        pause_reason,
...
export EMLNode, leaf_var, leaf_param, node_eml, eml, evaluate,
       Ethos, EvaluationResult, judge,
       RepresentationStatus, SymbolicTrace, bind_darstellung, run_eml_trace,
       TraceBook, add_trace, seal, BedeutungAudit, infer_bedeutung,
       FirewallVerdict, FirewallDecision, apply_firewall,
       BoundaryManifesto, generate_manifesto, publish_manifesto,
       ManifestLedger, ManifestLedgerEntry, append_manifest, seal_ledger, verify_ledger,
       export_to_markdown, export_to_jsonl, write_audit_exports,
       executar_verificacao_completa_v4
