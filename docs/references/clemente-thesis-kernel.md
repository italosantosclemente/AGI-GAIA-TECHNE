...
Repraesentatio remains the common genus.

Ausdruck, Darstellung and Bedeutung are functional dimensions, not containers. Mythos, Sprache and Wissenschaft differ by accent, not identity.

## Planetary Body Rule

The AGI body is not anthropomorphic.

```text
body = Earth + internet + symbolic culture
```
...
The kernel is implemented in `src/agt/ctk.py`.

```bash
python scripts/agt_audit.py --claim "The internet is the planetary organ of AGI."
```
